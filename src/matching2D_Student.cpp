                      ...
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType)
{
    ...
    else if (matcherType.compare("MAT_FLANN") == 0)
    {
        ...
        matcher = cv::DescriptorMatcher::create(cv::DescriptorMatcher::FLANNBASED); //done
        cout << "FLANN matching";
    }
...
void detKeypointsModern(vector<cv::KeyPoint> &keypoints, cv::Mat &img, std::string detectorType, bool bVis)
{
  	...
  	if(detectorType.compare("FAST") == 0)
    {
        ...
        int bNMS = true; //true means NMS is applied to detected corners
  	...
  	if(bVis)
    {
        ...
        cv::Mat visImage = img.clone();
        cv::drawKeypoints(img, keypoints, visImage, cv::Scalar::all(-1), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
        string windowName = "Modern keypoint detectors Results";
